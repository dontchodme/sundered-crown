 ...
 '''  /* ==== GRASP: ONE HAND, LARGE, AND TETHERED TO THE ARTIFACT ============
  ...
  drawGrip(m, over){
    for (const f of [m.a, m.b]){
      ...
      if (!over){
        /* THE GROUND. A DARK pool under the wielder, because the arm has to
           look like it came out of somewhere, and a thin ring at `radius`,
      ...
      /* ---- THE TETHER ---------------------------------------------------
         A limb of paired bones from the shell to the wrist, sagging when the
         hand is only reaching and pulled STRAIGHT when it has hold of
         something. The sag is the tension, and tension is the second of the
      ...
      for (let i = 1; i <= SEG; i++){
        const s = i / SEG, k = 1 - s;
        const qx = k * k * f.x + 2 * k * s * mx + s * s * wx;
        const qy = k * k * f.y + 2 * k * s * my + s * s * wy;
        /* PAIRED, like the radius and the ulna `_boneParts` opens with —
           which is the discovery that made Revenant's hands bone in the first
           place. The offset closes toward the wrist so the limb tapers. */
        const na = Math.atan2(qy - py, qx - px) + Math.PI / 2;
        const off = HR * 0.19 * (1 - s * 0.55);
        bone.push({ x1: px + Math.cos(na) * off, y1: py + Math.sin(na) * off,
                    x2: qx + Math.cos(na) * off, y2: qy + Math.sin(na) * off,
                    w: HR * 0.115 * (1 - s * 0.35) });
        bone.push({ x1: px - Math.cos(na) * off, y1: py - Math.sin(na) * off,
                    x2: qx - Math.cos(na) * off, y2: qy - Math.sin(na) * off,
                    w: HR * 0.095 * (1 - s * 0.35) });
      ...
      c.save();
      this._drawBones(c, bone, HR, P, crush ? 1.1 : 0.7);
      c.restore();
      ...
      /* ---- THE COUNT, ON THE TETHER -------------------------------------
         ...
         beside them is drawn in -- laid on the same curve, and photographed
         off a real match they were INVISIBLE: a bright dot on a bright line
         reads as a thicker line. So they cross the limb instead of following
         it, they are hot white against its purple, and an unlit one is a dark
         bar rather than an absence. Four ways of not being the tether.

         WHETHER A VIEWER CAN READ THE COUNT AT ALL IS §7b AND IT IS RICK'S.
         Nothing measures it and nothing can. */
      const grabs = G ? G.grabs : u.n;
      c.save();
      c.lineCap = "round";
      for (let i = 0; i < u.n; i++){
        const s = (i + 1) / (u.n + 1), k = 1 - s;
        const qx = k * k * f.x + 2 * k * s * mx + s * s * wx;
        const qy = k * k * f.y + 2 * k * s * my + s * s * wy;
        /* the tangent, from the curve's own derivative -- so a rung is square
           to the limb however the limb happens to be bent */
        const tx = 2 * k * (mx - f.x) + 2 * s * (wx - mx);
        const ty = 2 * k * (my - f.y) + 2 * s * (wy - my);
        const tl = Math.hypot(tx, ty) || 1;
        const nx = -ty / tl, ny = tx / tl;
        const lit = i < grabs;
        const half = HR * (lit ? 0.34 : 0.24);
        c.globalCompositeOperation = lit ? "lighter" : "source-over";
        c.globalAlpha = fade * (lit ? 1 : 0.75);
        c.strokeStyle = lit ? "#FFF2FF" : A.dark;
        c.lineWidth = HR * (lit ? 0.10 : 0.075);
        c.shadowColor = A.core; c.shadowBlur = lit ? 12 : 0;
        c.beginPath();
        c.moveTo(qx - nx * half, qy - ny * half);
        c.lineTo(qx + nx * half, qy + ny * half);
        c.stroke();
      }
      c.restore();
