#!/usr/bin/env python3
"""A COUNT INSTEAD OF A CLOCK — Rick's question, and it is the better mechanic.

    python3 vent_count_lab.py --game ../02-chain/sc-nightfell.html
...
game.** A cast spent near a wall tears twice what a cast spent in the middle
tears, so two casts of the same ultimate in the same fight are not the same
ultimate. A count removes that entirely — every cast is worth exactly n holes.

It also gives the thing a number a viewer can hold. GRASP's fifth grab is the
precedent and it is one build old.

THE SAFETY CLAUSE IS NOT OPTIONAL. A count with no clock behind it never ends
if the ball never reaches a wall, and the window would carry into the next cast
and out of the fight. So `dur` stops being the window and becomes the cap, and
this lab reports HOW OFTEN THE CAP IS WHAT ENDED IT — which is the number that
decides whether n is reachable at all.

  [1] THE PASS, unchanged.
  [2] n FROM 3 TO 8 AGAINST THE CLOCK, and at four blades, because a mechanic
      that lands the relic at a heavier blade is worth having for its own sake:
      a dwarven scythe that has to be filed down to 15 to be fair is not a
      dwarven scythe.

THIS IS A LAB AND NOT A BUILD. Thornwake stands in for Cindercleave, pulled
from the foe field, its Bramblesnare replaced, foes keeping their own
ultimates. Damage through `hurt` scaled by `dmgTakenMul`, never `resolveHit`,
so every lift is a FLOOR. Injection is runtime-only. NOTHING is written to any
build.
"""
...
LAB_JS = r"""([donor, foes, seeds, secs, cfg]) => {
  ...
  M.fireUlt = function (f, foe) {
    ...
    f.ultsFired++;
    f.ultVent = { t: 0, dur: cfg.dur, pass: null, left: cfg.nVents || 1e9, byCap: 0 };
  ...
  M.step = function (dt) {
    ...
    for (const f of [this.a, this.b]) {
      ...
      if (!V) continue;
      V.t += dt;
      const done = V.t >= V.dur || !f.alive || V.left <= 0;
      if (V.t >= V.dur && V.left > 0 && f.alive) m._capEnded++;
  ...
  M._closePass = function (f, P) {
    ...
    if (f.ultVent) { f.ultVent.left--; m._perCast.push(1); }
    ...
    /* RICK'S RULE: every hole fires INTO the room. A bearing with no inward
       component is dropped, which removes exactly the two that run along the
       wall the hole was torn from -- the two the even-eight arm was spending
       on empty stone. Three bearings a wall, four walls, all eight compass
       directions present in the game. */
    if (cfg.dirs === "inward") pick = DIRS.filter(d => d[0] * nx + d[1] * ny > 0.01);
  ...
  try {
    for (const f of foes) for (const s of seeds) {
      ...
      m._pen = []; m._dwell = []; m._k = []; m._capEnded = 0; m._perCast = [];
      let steps = 0;
...
FLOOR_JS = r"""([donor, foes, seeds, secs, dmg]) => {
  const DT = AC.CONFIG.physics.dt;
  const w  = AC.WEAPONS.find(x => x.id === donor);
  const saved = { aff: w.aff, ch: w.ult.charge, dmg: w.dmg,
                  onHit:  w.onHit  ? JSON.parse(JSON.stringify(w.onHit))  : null,
                  onSelf: w.onSelf ? JSON.parse(JSON.stringify(w.onSelf)) : null };
  ...
  try {
    ...
    w.aff = saved.aff; w.ult.charge = saved.ch; w.dmg = saved.dmg;
    delete w.onHit; delete w.onSelf;
    if (saved.onHit)  w.onHit  = saved.onHit;
    if (saved.onSelf) w.onSelf = saved.onSelf;
  }
...
CENTRE = dict(charge=15, dur=8.0, maxVents=8, ventLife=9.0, period=1.1,
              warm=0.35, beamDmg=9.0, half=14.0, sunderN=1,
              passMax=1.2, minPen=0.0, sizeOn=True, kMin=0.5, kMax=1.5,
              drive="width,life", speed=1100.0, taper=True, dirs="inward",
              dmg=None, nVents=0)
...
def summarize(rows):
    ...
    return {"win": mean(r["win"] for r in fin),
            ...
            "capEnded": mean(r["capEnded"] for r in rows),
...
def main():
    ...
    arms = [("clock 8s, blade 31.35", dict(dur=8.0)),
            ("aim: even eight", dict(dur=8.0, dirs="eight")),
            ("aim: straight only", dict(dur=8.0, dirs="perp")),
            ("size flat", dict(dur=8.0, sizeOn=False))]
    for n in (3, 4, 5, 6):
        for d in (31.35, 26.0):
            arms.append((f"n={n}, blade {d:g}",
                         dict(nVents=n, dur=14.0, dmg=None if d > 31 else d)))
    for d in (24.0, 22.0, 20.0, 19.0, 18.0, 16.0):
        arms.append((f"n=5, blade {d:g}", dict(nVents=5, dur=14.0, dmg=d)))
    ...
    with game(game_path=gp) as (page, errors):
        ...
        fl = page.evaluate(FLOOR_JS, [a.donor, foes, seeds, a.secs, None])
        ...
        for name, over in arms:
            ...
            print(f"    {name:<22}{rec['passes']:>8.1f}{rec['tears']:>7.1f}"
                  ...
                  f"{(rec['tears']/max(0.01,rec['casts'])):>10.2f}"
                  f"{rec['capEnded']:>8.2f}")

        n5 = out["n=5, blade 31.35"]
        clk = out["clock 8s, blade 31.35"]
        check("a count delivers a fixed number of holes a cast where the clock "
              "does not",
              abs(n5["tears"] / max(0.01, n5["casts"]) - 5) < 0.8,
              f"clock {clk['tears']/max(0.01,clk['casts']):.2f} a cast, "
              f"n=5 {n5['tears']/max(0.01,n5['casts']):.2f}")
        check("n is REACHABLE — the 14s safety cap is not what ends the window",
              n5["capEnded"] < 0.5,
              f"{n5['capEnded']:.2f} windows a fight ended on the cap")
        check("MORE HOLES IS STRONGER AT A FIXED BLADE — Rick's question, and "
              "the answer is yes",
              out["n=6, blade 31.35"]["win"] > out["n=5, blade 31.35"]["win"]
              > out["n=4, blade 31.35"]["win"] > out["n=3, blade 31.35"]["win"],
              " < ".join(f"n={n} {out[f'n={n}, blade 31.35']['win']:.1%}"
                         for n in (3, 4, 5, 6)))
        check("Rick's aim rule beats the even eight",
              clk["lift"] > out["aim: even eight"]["lift"],
              f"inward {clk['lift']:+.1%}, even eight "
              f"{out['aim: even eight']['lift']:+.1%}, straight only "
              f"{out['aim: straight only']['lift']:+.1%}")
        band = [k for k, v in out.items()
                if k.startswith("n=5, blade") and 0.47 <= v["win"] <= 0.53]
        check("the ladder brackets the field at n=5", bool(band),
              ", ".join(f"{k} {out[k]['win']:.1%}" for k in band) or
              "no arm inside 47-53%")
