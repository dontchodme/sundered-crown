  ...
  tickWire(dt){
    ...
    for (const [f, foe] of [[this.a, this.b], [this.b, this.a]]){
      ...
      if (W.caught){
        ...
        W.held += dt;
        /* THE HOLD, REFRESHED. There is no duration field for this and that
           is the design: the wire holds until the head comes around. The
           refresh is comfortably longer than a frame so the hold does not
           flicker, and it is torn down explicitly at the connect and at the
           window's end rather than being allowed to run out on its own -- a
           residual hold after the payoff would be the wire still gripping
           something the hammer has already thrown. */
