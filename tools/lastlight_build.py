...
UNDER_NEW = UNDER_ANCHOR + '''
    ...
    if (u.w === "lastlight"){
      ...
      if (u.phase === "cast"){
        ...
        /* CANDIDATE 3, SHORT -- the flash is unchanged, the HANG is not. It
           decayed over 1.20s after 0.30, so the wash sat on the arena for
           most of a 1.7s set-piece. Measured full-frame at the cap, the last
           sampled frame goes from 0.254 arena mean to 0.099. */
        const fade = 1 - clamp((u.t - 0.18) / 0.50, 0, 1);
        /* CANDIDATE 1, REACH, AND IT IS THE ONE THAT MATTERS.
         *
         * `86 + n * 26` is LINEAR in the count, which is 398px at scythes:12
         * -- a white radial disc, `lighter`, most of the arena. Rick watched
         * it and said the bloom was too intense on some ults; on this one he
         * was right and it is the only relic so far where he was. Measured at
         * the cap: arena mean 0.3956 with +0.0628 of that made by the post
         * chain, the largest bloom contribution in the game.
         *
         * Sublinear instead. The count still reads in the disc's size, and it
         * still reads in the scythes, which are what the count is actually
         * FOR. 0.3956 -> 0.2205 arena, and the bloom's own share halves to
         * +0.0356 -- the only one of three candidates that moved that number.
         *
         * WHY REACH AND NOT ALPHA: candidate 2 thinned the gradient's plateau
         * and the bloom did not notice -- +0.0627 against +0.0628. The bloom
         * reads the emissive layer and the core is white either way, so alpha
         * is invisible to it and reach is not. That generalises past this
         * relic. */
        const rad = (86 + 130 * Math.sqrt(n / 12)) * (1 - Math.pow(1 - ex, 2.4));
