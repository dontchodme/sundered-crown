#!/usr/bin/env python3
"""BLOODMIRROR'S BLADE. A CURVE FIRST, THEN A WIDE DIRECT MEASUREMENT.

    python bloodmirror_sweep.py --game ../02-chain/sc-bloodletting.html --only 0
    python bloodmirror_sweep.py --game ../02-chain/sc-bloodletting.html --only 1

`dmg 21` is the brief's STARTING POINT and not its answer
(`06-docs/v59/bloodmirror-build-brief-v59.md` section 5, which says to expect
20-22 and measured its curve on a 27-relic roster with Thornwake standing in for
a relic that did not exist). This settles it on the pin at 32.

**AND THE BUILT RELIC IS STRONGER THAN THE ONE THAT WAS PRICED.**
`bloodletting_relic_probe [12]` measures 13.12 ticks a fight against a
registered 10.5-11.0 -- 39.4 spectre damage a fight against the design's ~32
target -- which under the design's own one-scalar law
(`lift = +5.6 + 0.245 x spectre damage`) is about +1.8pp of extra lift for the
blade to absorb. So do NOT assume the answer is under 21 and do not assume it is
over: measure it.
...
itself refuted: it was sample size.

**AND THIS RELIC'S SURFACE IS NOT SIMPLE, WHICH IS WHY SECTION [0] IS A CURVE.**
`dmg` moves the blade AND the curse-free damage pool AND -- through the raised
ceiling -- what a blade blow is worth DURING the window, because the blade feeds
the ceiling too (Rick's ruling, design section 3.3). v51 section 4.5's
superlinear warning applies here where it did not to Shroudmaul, and
Gravemourn's curve BENT: 67.3% at dmg 47.2 and 60.6% at 52.0, because bigger
blows threw the quarry out of reach of a weapon that lands 5.6 times a fight.
...
`bloodmirror_build.TUNED_BM` and stage 4 writes it.
"""
...
def main():
    ...
    ap.add_argument("--curve", default="14,17,19,21,23,26",
    ...
    with game(game_path=gp) as (page, errors):
        ...
        print(f"\nBLOODMIRROR BLADE -- {gp.name}   {len(foes)} foes   "
