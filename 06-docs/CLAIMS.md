...
| umbral × bow | Crossweave, **THE CARRY** — stages 7-12 (held trio, nova, drawn explosion, voice, card line) existed only in `sc-crossweave.html` and were NOT in `sc-bloodletting.html`, which is what the app loads. Re-applied on top of the tip as a NEW link so Duskreave's base does not move under it. Rick, 2026-09-02: *"can you push your stuff in while duskreve runs?"* | Claude Code | 2026-09-02 | BUILDING | `tools/gloamwire_build.py` stages 7-12 |
