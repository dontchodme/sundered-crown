...
| **HIT STOP, Thornshear** | THE WINNOWING'S KUNAI carry a freeze that grows with the rung — 0 fresh, 0.02/0.04/0.06 off one/two/three walls, kills keep `killStop`. Rick, 2026-09-03: hit stop on loud many-hit ults *"reads as lag even though its probably not"*, seen on Thornshear; cause confirmed in `sc-minute` (every kunai buys the default ~50ms freeze for 2-4 damage; `spawnKunai` passes no `over`). Two edits, one relic. **BRIEF: `06-docs/v67/WINNOWING-HITSTOP-BRIEF-v67.md`.** Lands BEFORE the engine row below. | Cowork | 2026-09-03 09:24 | RULED, NOT BUILT — CODE'S TO BUILD | `06-docs/v67/WINNOWING-HITSTOP-BRIEF-v67.md` |
