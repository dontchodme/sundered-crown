...
02-chain/sc-minute.html          THE MINUTE, AND A BRANCH OFF sc-lastthree.
                                                   ...
                                                   THAT CHAIN IS GONE, scrapped
                                                   by Rick on 2026-09-02, so
                                                   this is simply the newest
                                                   link. 33 relics, unchanged
                                                   ...
                                                   SIXTY-SECOND FIGHT.
02-chain/sc-lastthree.html       sc-minute's PARENT, AND *NOT* THE BUILD OF RECORD.
...
> `sc-lastthree`** (and were missing from `sc-static`, which is gone) — the
> held trio, the nova, the drawn
...
twinblade was built to `06-docs/v64/`'s brief in three staged links, gated
green, and then dropped whole -- `sc-arclight`, `sc-storm` and `sc-static` are
deleted and **the chain tip is `sc-lastthree` at 33 relics again.** The cell is
OPEN and a replacement ultimate is Rick's and Cowork's, not Code's (rule 0).

> **NOTHING IS LOST AND THAT IS THE POINT.** The three links are in git at
> `4f022f4` and `tools/arclight_build.py --stage 1|2|3` rebuilds them
> byte-identically from `sc-lastthree`; `arclight_probe`, `arclight_price`,
> `arclight_sweep` and `_storm_pick` are all still in `tools/`. What was
> scrapped was a DECISION, not the work.

> **AND FOUR MEASUREMENTS SURVIVE IT, WHICH ARE THE EXPENSIVE PART.**
> `06-docs/v64/arclight-build-v64.md` is the record and every one of these
> outlives the relic that produced them:
>
> 1. **A RELIC THAT WINS OFF ITS ULTIMATE CANNOT BE BALANCED AT THE BLADE.**
>    Static read 93.9% at dmg 8.3 and **80.6% at dmg 0.5**, while the same body
>    with the ultimate stubbed won **0.0% of 330 fights** below dmg 4. The blade
>    gave back 13 points and needed 31. Design section 6's "has to give back ~40
>    points at the blade" failed, and it failed for the design's own model too
>    (D = 90.7%). **Sweep the floor curve BEFORE promising a blade will land a
>    relic.**
> 2. **TWO PAYOFFS ON ONE ULTIMATE CAN BE SUBSTITUTES RATHER THAN ADDENDS.**
>    Deleting the ward feed cost **3.6pp**; the ward feed ALONE, with the
>    detonation off, was worth **+29.2pp**. Either half won the fight on its
>    own, so **every one-knob curve understated its own knob** and any
>    single-number tune would have looked nearly free and changed nearly
>    nothing.
> 3. **A LAB THAT BOUNCES THINGS OFF THE ARENA IS NOT MEASURING THIS HALL.**
>    `storm_price` used `x < P.rb`; the engine's own projectiles use
>    `n = this.inset`, which the seals walk 0 -> 140. In the open hall the build
>    reproduced the priced swarm on all five counts and once the hall closed it
>    was five times that. **Any future overlay has to close the room.**
> 4. **AND SECTION 4.2b EARNED ITS PLACE AGAIN.** `storm_price.py` UNMODIFIED,
>    on the design's own build, on this machine, returned **+37.5** against a
>    published **+33.1** -- v64 was priced on Chromium 141 in a Cowork container
>    and this repo pins 151. A gate read against the published decimal called
>    the build 7.8pp hot; read against a reproduction control it was 3.4pp and
>    in band. **Every decimal in `06-docs/v64/` is on the other runtime.**
